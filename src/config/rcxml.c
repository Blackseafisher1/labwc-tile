...
entry(xmlNode *node, char *nodename, char *content)
{
	...
	} else if (!strcasecmp(nodename, "alwaysOnTop.overlay.snapping")) {
		set_bool(content, &rc.snap_overlay_always_on_top);
	...
	} else if (!strcasecmp(nodename, "adjustTiledNeighbors.resize")) {
		set_bool(content, &rc.resize_adjust_tiled_neighbors);
...
rcxml_init(void)
{
	...
	rc.snap_overlay_always_on_top = false;
	...
	rc.resize_adjust_tiled_neighbors = false;
